...
export function ProductCard({ product, index = 0, onAddToCart }: ProductCardProps) {
  ...
  return (
    <motion.div
      ...
      className="group relative bg-gradient-to-b from-matte-black/80 to-black rounded-xl overflow-hidden border border-primary-red/30 hover:border-primary-red/60 transition-all duration-300 flex flex-col cursor-pointer"
    >
      ...
      >
        <div 
          ...
          onClick={(e) => {
            e.stopPropagation();
            window.dispatchEvent(new CustomEvent('open-image-lightbox', { detail: { imageUrl: product.image } }));
          }}
        >
          <motion.img
            src={product.image}
          ...
          <div 
            className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end justify-between p-4" 
            onClick={(e) => {
              e.stopPropagation();
              window.dispatchEvent(new CustomEvent('open-image-lightbox', { detail: { imageUrl: product.image } }));
            }}
          >
            <motion.button
              ...
              className="bg-primary-red/80 hover:bg-primary-red p-3 rounded-full transition-colors duration-300 relative z-20"
            >
      ...
      <div className="p-5 relative z-10 flex-grow flex flex-col justify-between" onClick={(e) => e.stopPropagation()}>
        <div>
          <h3 
            onClick={() => window.dispatchEvent(new CustomEvent('open-product-details', { detail: { productId: product.id } }))}
